...
const init = async () => {
    ...
    const lambdaName = `slack-standup-${process.env.stage}-worker`; // This is from serverless.yml
    ...
    app.view("standup_view", async ({ack, body, view, client, logger, }) => {
        ...
        const obj = initEvent;
        obj.path="/worker/events";
        obj.resource="/worker/events";
        obj.requestContext.path="/worker/events";
        obj.requestContext.resourcePath="/worker/events";
        try {
            await context.lambdaClient.send(new InvokeCommand({
                FunctionName: lambdaName,
                LogType: LogType.Tail,
                InvocationType: InvocationType.Event,
                // Payload: Buffer.from(JSON.stringify(createLambdaRequest(body))),
...
function createLambdaRequest(payload: any){
