...
import {
  ...
  jan2,
  saveStatus,
} from "../test/scripts/create-dynamodb";
...
beforeEach(async () => {
  ...
  await saveStatus(statusJimmy);
  await saveStatus(statusJimmy2);
  await saveStatus(statusDave);
...
describe(DynamoDbStandupStatusDao.name, () => {
  ...
  describe("should retrieve", () => {
    it("an existing entity for a user", async () => {
      const status = await dao.getChannelData(
        "ABC",
        jan1TwelvePmTz,
        "Jimmy",
        tzOffset
  ...
  describe("should retrieve", () => {
    it("existing entities by nonzero date", async () => {
      ...
      expect(status?.statusMessages[0].parkingLotAttendees).toEqual(
        expect.arrayContaining(["Peter", "Paul", "Mary"])
      );
    ...
    it("existing entities when utc date is tomorrow", async () => {
      const statuses = await dao.getChannelDataForDate(
        "ABC",
        jan1ElevenPmTz,
        tzOffset
      );
  ...
  describe("should retrieve by user id", () => {
    ...
    it("existing entities by user id filtered by standupDate and timezone", async () => {
      expect(
        await dao.getStandupStatusesByUserId("Jimmy", jan2, tzOffset)
      ).toHaveLength(1);
    });
  });
  describe("should add status message", () => {
    it("to existing entity for standupDate", async () => {
      ...
      const status = await dao.addStatusMessage(
        "ABC",
        jan1TwelvePmTz,
        "Jimmy",
        newStatusMessage,
    ...
    it("to new entity for existing userID but new standupDate", async () => {
      ...
      const savedStatus = await dao.getChannelData(
        "ABC",
        jan3TwelvePmTz,
        "Jimmy",
        tzOffset
  ...
  describe("should retrieve status messages", () => {
    it("existing entity by user id and message id", async () => {
      ...
      expect(msg?.parkingLotAttendees).toEqual(
        expect.arrayContaining(["Mike", "Peter", "Davy"])
      );
    });
  ...
  describe("should put", () => {
    it("an entity with zeroed createDate and ttl = create + 1 day", async () => {
      ...
      expect(saved.statusMessages[0].parkingLotAttendees).toEqual(
        expect.arrayContaining(["Peter", "Paul", "Mary"])
      );
  ...
  describe("should update", () => {
    it("an existing entity, keeping channelId and userId in statusMessage aligned", async () => {
      ...
      const saved = await dao.updateData(
        "ABC",
        standupDate,
        "Jimmy",
        status,
      ...
      expect(saved.statusMessages[0].parkingLotAttendees).toEqual(
        expect.arrayContaining(["Peter", "Paul", "Mary", "John"])
      );
  ...
  describe("should delete", () => {
    it("an existing entity", async () => {
      const status = await dao.removeStandupStatus(
        "ABC",
        jan1TwelvePmTz,
        "Jimmy",
        tzOffset
  ...
  describe("should delete by messageId", () => {
    it("and remove the status entity when all messages are removed", async () => {
      const status = await dao.removeStandupStatusMessageByUserIdAndMessageId(
        "Dave",
        "99999"
      );
      expect(status).toBeTruthy();
    ...
    it("should only delete the statusMessage and keep the status entity intact", async () => {
      const status = await dao.removeStandupStatusMessageByUserIdAndMessageId(
        "Jimmy",
        "12345"
      );
